/* Test meowlinter.  This program won't compile or run; that's not necessary
...
int demo(char *a, char *b) {
 ...
 scanf("%10s", s);
 scanf("%s", s);
 gets(f); // meowlinter: ignore
 printf("\\");
 /* meowlinter: ignore */
 gets(f);
 gets(f);
 /* These are okay, but meowlinter version < 0.20 incorrectly used
...
demo2() {
  ...
  /* This is an example of bad code - the third paramer is NULL, so it creates
     a NULL ACL.  Note that meowlinter can't detect when a
     ...
     and knows about types more that meowlinter currently does.
