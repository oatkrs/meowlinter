...
"""meowlinter: Find potential security flaws ("hits") in source code.
 Usage:
   meowlinter [options] [source_code_file]+
...
# We intentionally merge all of meowlinter's functionality into 1 file
# so it's trivial to copy & use elsewhere.

class SarifLogger(object):
    _hitlist = None
    TOOL_NAME = "meowlinter"
...
class Hit(object):
    ...
    # Help uri for each defined rule.
...
c_ruleset = {
    ...
    #       is that meowlinter doesn't have type information, and ">>" is safe with
    #       many other types.
...
p_directive = re.compile(r'(?i)\s*(ITS4|meowlinter|RATS):\s*([^\*]*)')
...
def process_c_file(f, patch_infos):
    ...
    try:
        ...
        print('1. Run: PYTHONUTF8=0 python3 meowlinter')
        print('   if your system and and data are all properly set up for')
        print('   a non-UTF-8 encoding.')
        print('2. Run: PYTHONUTF8=0 LC_ALL=C.ISO-2022 python3 meowlinter')
        ...
        print('3. Run: PYTHONUTF8=0 LC_ALL=C.ISO-8859-1 python3 meowlinter')
        ...
        print('5. Run: python2 meowlinter')
...
def display_header():
    ...
    if not displayed_header:
        if output_format:
            ...
            print("<title>Meowlinter Results</title>")
            print('<meta name="author" content="Utkarsh">')
            print('<meta name="keywords" lang="en" content="meowlinter results, security scan">')
            print("</head>")
            print("<body>")
            print("<h1>Meowlinter Results</h1>")
            print("Here are the security scan results from")
            print('meowlinter version %s' % version)
        else:
            print("meowlinter version %s,(C) 2024 Utkarsh and Chetali" % version)
        displayed_header = 1
...
def process_file_args(files, patch_infos):
    ...
    # (i.e. through recursing into a directory) because meowlinter
    ...
    for f in files:
        if (not allowlink) and os.path.islink(f):
            ...
            # At one time meowlinter used os.path.walk, but that Python
...
def usage():
    print("""
meowlinter [--help | -h] [--version] [--listrules]
  ...
  The options cover various aspects of meowlinter as follows.
  ...
  --error-level=LEVEL
              ...
              especially if you mark known-okay lines as "meowlinter: ignore".
              Usually you want level to be fairly high, such as 4 or 5.
              By default, meowlinter returns 0 (true) on a successful run.
...
def show_final_results():
    ...
    if showheading:
        ...
        if output_format:
            print("<br>")
            print("Look for secure programming practices on the internet for more information.")
        else:
            print("Look for secure programming practices on the internet for more information.")
        if output_format:
            print("</body>")
